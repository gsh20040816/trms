...
- 处理逻辑：
  ...
  - 对扫描 PDF、图片和截图，优先通过支持图像输入的 VLM API 直接提取结构化信息，而不是把“先落传统 OCR 文本”写成唯一必经路径；
  - 对置信度不足的信息标记为待确认；
  - 允许成员或管理员人工更正。
- 输出：
  - 结构化材料元数据；
...
| 报销任务 | 任务状态 | 枚举 | 是 | 草稿、收集中、复核中、可导出、已完成 |
| 成员 | 学号 | 文本 | 是 | 成员唯一标识之一 |
| 成员 | 姓名 | 文本 | 是 | 用于展示和确认 |
...
| FR-007 费用归属与多人分摊 | 部分完成 | 已实现发票费用分摊 API、分摊成员必须属于任务成员名单、分摊金额合计必须等于发票金额、重复成员输入校验；尚未联动成员确认和管理员修改后的重新确认 |
| FR-008 成员费用确认 | 部分完成 | 已实现成员对本人费用分摊明细确认或提出异议、异议原因必填、禁止确认他人明细、确认记录查询；尚未接入真实用户认证和管理员修改金额后的确认失效机制 |
