...
1. 对 PDF、图片、截图和辅助材料进行 PDF 文本提取，以及基于支持图像输入的 VLM API 的直接结构化提取。
...
1. 优先从文本 PDF 中提取可复制文本，减少图像调用成本和误差。
2. 对扫描 PDF、图片和截图，优先通过 VLM API 直接输出结构化结果；如需引入传统 OCR，只能作为补充路径，而不是新的唯一主路径。
3. VLM / LLM 输出必须经过 JSON Schema 或 Pydantic 模型校验。
4. 图像提取失败时保留失败原因，不生成虚假字段。
5. 识别结果必须带字段来源和置信度。
