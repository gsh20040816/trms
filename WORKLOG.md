...
### 完成内容
- 根据用户补充说明，已确认 [data/6.png](/home/gsh/workspace/TRMS/data/6.png) 指向的不是系统管理员页，而是管理员首页总览概览卡的 UI 问题。
- 已修改 [web/src/app/admin-task-list.tsx](/home/gsh/workspace/TRMS/web/src/app/admin-task-list.tsx)：
  - 将概览卡提示文案统一收短，避免在当前卡片宽度下被拆成两行；
  - 把提示徽标恢复为单行展示，不再允许提示文本自动换行把卡片头部挤坏；
  - 当前概览卡提示已收口为：
    - `先补齐再发布`
    - `成员继续上传`
    - `优先清异常`
    - `缺件或逾期`
    - `直接整理导出`
- 这样首页五张概览卡在同一行时，标题与提示不会再互相抢宽度，也不会出现“草稿待完善”“收集中”等标题被压成竖排的异常布局。

### 根因
- 管理员首页概览卡原先使用较长的提示文案，例如“补齐成员与费用后再发布”；
- 当五张卡片同排显示时，右侧徽标文字占宽过大，直接压缩了左侧标题区，导致标题按单字换行，形成截图中的异常竖排效果。

### 风险与影响面
- 本轮只收口管理员首页概览卡的提示文案与单行展示方式，不改任务列表排序、优先任务计算或筛选逻辑。
- 当前解决策略是“文案收短 + 单行徽标”，不是继续扩卡或改整页网格；这样改动最小，但如果后续还要塞更多提示信息，就需要重新设计该卡片的信息密度。
