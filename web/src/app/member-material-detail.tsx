...
function resolveMaterialPageConfig(item: TaskMemberWorkbenchItem) {
  return resolveNonInvoiceMaterialDetailConfig(
    ...
    item.recognition?.recognized_fields ?? null,
  );
}
  ...
  return (
    <RoleWorkspace
      header={(
        ...
        description={pageConfig?.description ?? "按当前材料类型展示对应识别字段和处理动作。"}
        actions={(
          <Button
            component={Link}
    ...
    >
      ...
      {detailState.status === "ready" && item && pageConfig ? (
        <>
          ...
          <SectionCard title="识别判断" description="先看系统对材料类型和费用归类的初步判断，再决定是否需要修正。">
          ...
          <SectionCard title="关联归属发票" description="勾选这份材料实际关联的发票并保存，处理完后会自动刷新结果。">
            {pendingLinkageItem ? (
              <>
                {pendingLinkageItem.linked_invoices.length > 0 ? (
                  <div className="page-stack">
                    <p className="field-hint">已关联发票：</p>
                ...
                {linkageInvoiceOptions.length > 0 ? (
                  <>
                    ...
                    <div className="inline-actions">
                      ...
                      >
                        {savingLinkageChanges ? "保存中..." : "保存归属变更"}
                      </Button>
                    </div>
                  </>
                ) : (
              ...
              <p className="field-hint">当前材料的归属已经明确，这里不再重复显示可编辑候选。</p>
            )}
          </SectionCard>
        </>
      ) : null}
